...
function getGeminiClient(): GoogleGenAI {
  if (!aiInstance) {
    ...
    if (!apiKey || apiKey === "MY_GEMINI_API_KEY" || apiKey.trim() === "") {
      throw new Error("GEMINI_API_KEY environment variable is missing or empty. Please set your Gemini API Key in the Settings > Secrets configuration panel.");
    }
...
app.post("/api/audit", async (req, res): Promise<any> => {
  try {
    ...
    if (!text && !fileData) {
      return res.status(400).json({ error: "Please provide either written logs text or an uploaded image of the notebook page." });
    }

    // Lazy load Gemini Client to handle cases where API key isn't provided gracefully inside the UI rather than crashing the process at startup
    let ai;
    try {
      ai = getGeminiClient();
    } catch (err: any) {
      return res.status(500).json({
        error: err.message || "GEMINI_API_KEY is missing.",
        isConfigError: true
      });
    }
...
3. AUDIT RECONCILIATION & NOTES:
   ...
   - Summarize daily insights in notes. Return the results in the requested exact JSON structure.`;
    ...
    const logPrompt = text ? `Analyze the following refilling station daily log:\n\n${text}` : "Analyze the uploaded photograph of a handwritten refilling station notebook log.";
    ...
    const responseSchema = {
      ...
      properties: {
        date: {
          type: Type.STRING,
          description: "The date of the daily logs in YYYY-MM-DD format. If not explicitly found, infer from content or use the current year/season context."
        },
        meter_reading: {
          type: Type.OBJECT,
          properties: {
            opening: { type: Type.NUMBER, description: "The initial water meter reading of the day" },
            closing: { type: Type.NUMBER, description: "The ending water meter reading of the day" },
            total_consumed_m3: { type: Type.NUMBER, description: "Calculated Closing Reading - Opening Reading" }
          },
        ...
        financial_summary: {
          type: Type.OBJECT,
          properties: {
            total_gallons_sold: { type: Type.INTEGER, description: "Total quantity of gallons sold across all log items" },
            pure_water_sales: { type: Type.NUMBER, description: "Total value of base water sold, strictly valued at their standard rate (gallons * standard_rate)" },
            miscellaneous_revenue: { type: Type.NUMBER, description: "Total value of isolated extra fees (such as cylinder container purchases, deposits, delivery costs)" },
            grand_total_cash_expected: { type: Type.NUMBER, description: "Total cash expected (pure_water_sales + miscellaneous_revenue)" }
          },
        ...
        customer_breakdown: {
          ...
          items: {
            type: Type.OBJECT,
            properties: {
              name: { type: Type.STRING, description: "Name/Description of the customer" },
              gallons: { type: Type.INTEGER, description: "Amount of gallons of water bought" },
              total_paid: { type: Type.NUMBER, description: "Total payment received from the customer" },
              inferred_rate_per_gallon: { type: Type.INTEGER, description: "Determined standard base water rate (should be strictly either 20 or 25)" }
            },
        ...
        audit_errors_or_notes: {
          ...
          description: "List of mathematical audit errors, notes about underpayments, pricing deviations, container upgrades, or estimated water loss percentage."
        }
      },
    ...
    return res.json(auditedData);

  } catch (error: any) {
    console.error("Auditing error: ", error);
    return res.status(500).json({
      error: error.message || "An unexpected error occurred during the financial audit process. Please try again."
    });
  }
});
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Water Refilling Station Audit Server running at http://localhost:${PORT}`);
  });
}
